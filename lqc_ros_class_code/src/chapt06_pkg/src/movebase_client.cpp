...
int main(int argc, char **argv)
{
    ...
    // 待发送的目标点 1 在 map 坐标系下的坐标位置（将上一节标定的位置填写到对应的 x，y，yaw）
    quaternion.setRPY(0, 0, 0); // 最后一个数对应 yaw 角，正前方向量顺时针旋转
    goal1.target_pose.pose.position.x = 1.12; //x，相对于建图起点前后的值，前正后负
    goal1.target_pose.pose.position.y = 1.05; //y，相对于建图起点左右的值，左正右负
    goal1.target_pose.pose.orientation.z = quaternion.z();
    goal1.target_pose.pose.orientation.w = quaternion.w();
    goal1.target_pose.header.frame_id = "map";
    goal1.target_pose.header.stamp = ros::Time::now();
    ac.sendGoal(goal1);
    ROS_INFO("Send Goal 1 !!!");ac.waitForResult();
    ...
    // 待发送的目标点 2 在 map 坐标系下的坐标位置
    quaternion.setRPY(0, 0, 0); // 最后一个数对应 yaw 角，正前方向量顺时针旋转
    goal2.target_pose.pose.position.x = 2.41;
    goal2.target_pose.pose.position.y = 1.96;
